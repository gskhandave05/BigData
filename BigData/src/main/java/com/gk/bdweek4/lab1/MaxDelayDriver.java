...
public class MaxDelayDriver extends Configured implements Tool {
	public static void main(String[] args) throws Exception {
	
		int exitCode = ToolRunner.run(new Configuration(),new MaxDelayDriver(), args);
	...
	public int run(String[] args) throws Exception {
		...
		if (args.length != 2) {
		    ...
		    job.setJarByClass(MaxDelayDriver.class);
		  ...
		  //Setting the type of the key-value pair to write in the output file.
		    ...
		    //Setting the grouping and sorting comparator classes
		      job.setGroupingComparatorClass(UniqueCarrierComparator.class);
		      job.setSortComparatorClass(CarrierDelayComparator.class);
		    
		    //Setting the custom partitioner
		  job.setPartitionerClass(MaxDelayPartitioner.class);
